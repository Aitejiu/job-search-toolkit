...
如果当前对话已经配置好了 vault 和飞书同步配置，日常使用不需要贴 skill 路径，也不需要说技术字段。直接像平时交代事情一样说即可：

```text
帮我把最近的投递记录同步到飞书多维表格，先让我看看准备同步哪些，别直接写入。
```

看过预览、确认没有问题后，再说：

```text
可以，写入吧。写完帮我确认一下有没有重复记录。
```

后续有新投递时，直接说：

```text
我刚又加了几条投递，帮我再同步一遍。新的加进去，已有的更新，别重复，也别删历史记录。
```

第一次使用或更换数据目录时，再补充位置即可：

```text
我的 Obsidian 在 /Users/你的用户名/obsidian/job-search，飞书同步配置在
求职/配置/飞书同步.md。以后投递同步都用这两个位置。
```

如果 Codex 没有自动识别本项目的 skill，只需在自然表达前加一句“用 `feishu-job-sync` 帮我处理”，不需要把 `SKILL.md` 的完整路径贴出来。
